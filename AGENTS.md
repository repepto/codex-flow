...
The `ask:<question>` command is always available as a read-only single-shot reasoning escape hatch, including during active, stale, blocked, dirty, or Strict Mode states. It must not create active steps, records, reports, commits, state updates, or workflow transitions, and it must not run `apply`, `adopt-step`, `discard-step`, or `resync`. After answering, the workflow state and git tree must remain unchanged.

Workflow-related command responses must append the Workflow State Footer defined in `.codex/core/commands.md`. Footer generation is read-only and must not create or modify steps, records, reports, commits, project files, workflow files, runtime state, or command transitions. The footer must reflect the current active step, goal, strict/discussion modes, git/workflow cleanliness, step-base state, recommended next command, available commands, and blocked commands without inventing state. Long `help`, `details`, and `ask:<question>` responses may use the compact footer.
