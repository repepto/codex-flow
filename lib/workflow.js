...
function inspectWorkflowFooterState(targetRoot) {
  ...
  return gateResult(true, [], uniqueStrings(warnings), {
    ...
    gitTree,
    stepBase,
    recommendedStep: commandState.recommendedNextCommand === 'ok' && nextStep.ok
      ? nextStep.details.recommendation
      : null,
...
function formatFullWorkflowStateFooter(details) {
  ...
  appendRecommendedStepLines(lines, details);
...
function formatCompactWorkflowStateFooter(details) {
  ...
  appendCompactRecommendedStepLines(lines, details);
  lines.push('Next:');
  lines.push('');
...
function appendRecommendedStepLines(lines, details) {
  if (!details.recommendedStep) {
    return;
  }
  lines.push('Recommended Step:');
  lines.push('');
  lines.push(details.recommendedStep);
  lines.push('');
}

function appendCompactRecommendedStepLines(lines, details) {
  if (!details.recommendedStep) {
    return;
  }
  lines.push('Step:');
...
function setStrictMode(targetRoot, value) {
  const mode = normalizeStrictMode(value);
  if (!mode) {
    return gateResult(false, ['Strict Mode value must be true or false.'], [], {});
  }

  const git = getGitState(targetRoot);
  if (!git.ok) {
    return gateResult(false, [git.error], [], { git });
  }
  if (isGitTracked(targetRoot, '.codex/state.md')) {
    return gateResult(false, ['.codex/state.md must be ignored runtime state before changing Strict Mode.'], [], { git });
  }
  if (!isGitIgnored(targetRoot, '.codex/state.md')) {
    return gateResult(false, ['.codex/state.md must be ignored runtime state before changing Strict Mode.'], [], { git });
  }

  const beforeGit = git;
  const state = readCodexState(targetRoot);
  const beforeFields = { ...state.fields };
  const previousMode = beforeFields['Strict Mode'] || 'true';
  const fields = {
    'Sync Backend': beforeFields['Sync Backend'] || 'git',
    'Last Known Revision': beforeFields['Last Known Revision'] || 'none',
    'Last Known Branch': beforeFields['Last Known Branch'] || 'none',
    'Last Sync Source': beforeFields['Last Sync Source'] || 'none',
    'Strict Mode': mode,
    'Discussion Mode': beforeFields['Discussion Mode'] || 'none'
  };
  ...
  return gateResult(true, [], [], {
    strictMode: mode,
    previousStrictMode: previousMode,
    changed: !state.exists || previousMode !== mode,
    created: !state.exists,
    beforeGit,
    state: readCodexState(targetRoot),
    currentStep: readCurrentStep(targetRoot)
  });
}
...
function isGitIgnored(targetRoot, relativePath) {
  return runGit(targetRoot, ['check-ignore', '-q', '--', relativePath]).status === 0;
}
